...
parser$add_argument("-hp2","--hierarchy_parameter2", type = 'character', help = "Specify secondary hierarchy parameter as listed in input hierarchy matrix (e.g., column name). Used to identify 2nd hierarchy column.", required=FALSE)
...
if (!is.null("args$hierarchy_parameter2")) {
  hp2 <- args$hierarchy_parameter1
}
...
if (!is.null(hierarchy_matrix)) {
  ...
  if ((hp1 %in% colnames(hierarchy_key)) & (hp2 %in% colnames(hierarchy_key))) {
    ...
    hp2_i <- which(colnames(hierarchy_key)==hp1)
    message(paste0("Hierarchy parameter ", hp2, " detected in column ", hp2_i,". Hierarchy matrix correctly formatted. Assessing integration into mutation matrix."))
  } else {
  stop(sprintf("Error: Input hierarchy parameters not found in hierarchy matrix. Please correct provided hierarchy parameter. Stopping mSigHdp pipeline."))
  }
} else {
