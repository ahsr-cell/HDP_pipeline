process HDP_flat_noprior {
    ...
    println(chain_index)
    """
    Rscript --vanilla ${projectDir}/bin/HDP_flat_noprior.R -a ${analysis_type} -b ${burnin_iterations} -o ${posterior} -i ${posterior_space} -cp ${concentration_parameter} -t ${threshold} -n ${chain_index} -c ${mutation_context} ${mutational_matrix}
    """
}